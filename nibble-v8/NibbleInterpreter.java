...
public class NibbleInterpreter {
    ...
    private void stmt_loop() {
        ...
        ArrayList<String> arraylist = new ArrayList<String>();
        mustBe(TokenKind.LPAREN);
        String code = new String(scanner.code);
        int pos = scanner.pos;
        arraylist.addAll(expression());
        mustBe(TokenKind.RPAREN);
        block();
        ...
        while (true) {
            ...
            NibbleScanner sc2 = new NibbleScanner(code.substring(pos));
            NibbleInterpreter int2 = new NibbleInterpreter(sc2);
            loop.addAll(int2.expression());
            System.out.println(loop.get(0));
            System.out.println(loop.get(1));
            break;
        }
    }
